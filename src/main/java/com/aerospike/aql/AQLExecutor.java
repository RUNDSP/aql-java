	...
	public void exitStat(StatContext ctx) {
		try {
			...
			if (ctx.QUERY() != null) {
				...
				//"sindex/test/original-index"
				//keys=0;objects=0;ibtr_memory_used=18688;nbtr_memory_used=0;si_accounted_memory=18688;load_pct=100;loadtime=0;stat_write_reqs=0;stat_write_success=0;stat_write_errs=0;stat_delete_reqs=0;stat_delete_success=0;stat_delete_errs=0;stat_gc_recs=0;stat_gc_time=0;query_reqs=0;query_avg_rec_count=0;query_avg_record_size=0;query_agg=0;query_agg_avg_rec_count=0;query_agg_avg_record_size=0;query_lookups=0;query_lookup_avg_rec_count=0;query_lookup_avg_record_size=0;gc-period=1000;gc-max-units=1000;data-max-memory=18446744073709551615;tracing=0;histogram=false;ignore-not-sync=true
				String nameSpace = ctx.namespace_name().getText();
				String indexName = ctx.index_name().getText();
				String infoString = info(String.format("sindex/%s/%s", nameSpace, indexName));
